...
def load_data(embeddings_path, labels_path):
    ...
    raw_embeddings = raw_embeddings.reshape(67, 424, 3107)
...
class EmbeddingTransformer(nn.Module):
    def __init__(self, input_dims=(23, 35, 3072), hidden_dim=36, output_dim=3,
                 num_encoder_layers=4, nhead=4, dim_feedforward=512, dropout=0.1):
        super().__init__()
        self.input_dims = input_dims

        # Initialize linear layers based on the provided input dimensions
        self.fc_layers = nn.ModuleDict()
        self.hidden_dim = hidden_dim
        self.total_hidden_dim = 0

        for idx, dim in enumerate(self.input_dims):
            if dim > 0:  # Skip unused dimensions (e.g., missing inputs)
                self.fc_layers[f'fc{idx+1}'] = nn.Linear(dim, hidden_dim)
                self.total_hidden_dim += hidden_dim

        self.pos_encoder = PositionalEncoding(self.total_hidden_dim)
        encoder_layer = nn.TransformerEncoderLayer(d_model=self.total_hidden_dim, nhead=nhead,
                                                   dim_feedforward=dim_feedforward, dropout=dropout)
        self.transformer_encoder = nn.TransformerEncoder(encoder_layer, num_layers=num_encoder_layers)
        self.fc_out = nn.Linear(self.total_hidden_dim, output_dim)
    ...
    def forward(self, x):
        ...
        # Process each input dimension dynamically
        for idx, dim in enumerate(self.input_dims):
            if dim > 0:  # Skip unused dimensions
                slice_x = x[..., start:start+dim]
                fc_layer = self.fc_layers[f'fc{idx+1}']
                feature_tensors.append(fc_layer(slice_x))
                start += dim

        # Concatenate processed features
        x_cat = torch.cat(feature_tensors, dim=-1)  # (seq_len, batch_size, 424, total_hidden_dim)
...
if __name__ == "__main__":
    embeddings_path = 'quang/time_series_prediction/ablation_experiments/embeddings/hf_airbnb_llm_embeddings.csv'
    ...
    # input_dims = (0,35,0) # hf_embeddings 1000 epochs
    # input_dims = (0,0,3072) # airbnb_llm_embeddings 150 epochs
    # input_dims = (23,35,0) # road_hf_embeddings 1000 epochs
    # input_dims = (23,0,3072) # road_airbnb_llm_embeddings  150 epochs
    input_dims = (0,35,3072) # hf_airbnb_llm_embeddings  200 epochs
    
    
    # input_dims = (0,35,35) # gat_hf_air_llm    gcn_hf_air_llm    1000 epochs
    # input_dims = (0,0,692) # airbnb_raw_embeddings   300 epochs
    # input_dims = (0,35,692) # hf_airbnb_raw_embeddings  300 epochs
    # input_dims = (23,0,692) # road_airbnb_raw_embeddings  300 epochs
    ...
    test_loader, last_model = train_model(full_embeddings, full_labels, embeddings_path=embeddings_path,
                                        ...
                                        epochs=epochs, batch_size=batch_size,lr=lr, device=device, input_dims=input_dims)
