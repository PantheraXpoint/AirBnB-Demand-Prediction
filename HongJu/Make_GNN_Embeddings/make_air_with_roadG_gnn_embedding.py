...
# road 임베딩이랑 순서 일치 시키기
def order_dong_to_road(air, road):
    road_dong_order = list(road['ADM_NM'].unique())
    
    group = air.groupby(['Reporting Month'])

    result = []
    for key, df in group:
            ex = df
            ex['ADM_NM'] = pd.Categorical(ex['ADM_NM'], categories=road_dong_order, ordered=True)
            ex = ex.sort_values(['ADM_NM'])
            result.append(ex)
            
    air_emb = pd.concat(result, ignore_index=True)
    return air_emb
...
def main():
    ...
    # 전체 기본 임베딩 불러오기
    air_emb = pd.read_csv('../../Dataset/AirBnB_LLM_New/llama3_no_listing_raw_fill_na_ver.csv')
    road = pd.read_csv('../../Dataset/Raw_Embeddings/Road_Embeddings.csv')
    
    # 매달 순서를 road와 일치 시키기
    air_emb = order_dong_to_road(air_emb, road)
    
    group = air_emb.groupby(['Reporting Month'])
    
    G_path = f'../../Dataset/Road_Graph/road_graph.gpickle'   # 그래프 경로
    edge_index, edge_weight = get_graph(G_path, road, 'ADM_NM')
    print(f'그래프 불러오기 성공!')
    ...
    for key, df in group:
        ...
        # 학습된 임베딩 저장
        gcn_path = "../../Dataset/GCN_Embeddings/GCN_air_llama_no_list_raw_with_roadG_Embeddings.csv"
        gat_path = "../../Dataset/GCN_Embeddings/GAT_air_llama_no_list_raw_with_roadG_Embeddings.csv"
