...
def order_dong_to_road(hf, road):
    ...
    hf['행정동코드'] = hf['행정동코드'].replace(dong_name_mapping)

    group = hf.groupby(['기준일ID'])

    result = []
    for key, df in group:
            ex = df
            ex['행정동코드'] = pd.Categorical(ex['행정동코드'], categories=road_dong_order, ordered=True)
            ex = ex.sort_values(['행정동코드'])
            result.append(ex)
            
    hf_emb = pd.concat(result, ignore_index=True)
    return hf_emb
...
def main():
    ...
    # 매달 순서를 road와 일치 시키기
    hf_emb = order_dong_to_road(hf_emb, road)
    ...
    for key, df in group:
        ...
        # 학습된 임베딩 저장
        gcn_path = "../../Dataset/GCN_Embeddings/GCN_Human_Flow_with_roadG_Embeddings.csv"
        gat_path = "../../Dataset/GCN_Embeddings/GAT_Human_Flow_with_roadG_Embeddings.csv"
